...
class Runs(dict):
    ...
    def __or__(self, runs) -> Runs:
        return Runs(dict(self) | dict(runs))

    def __ior__(self, runs) -> Runs:
    ...
    def df(self, max_level: int = 0) -> pd.DataFrame:
        ...
        #    [run.fields | {"id_run": run.id_run} for run in self.values()],
        #    max_level=max_level,
        # )
        df_list = []
        for run in self.values():
            fields_dict = run.fields.copy()
            fields_dict.update({"id_run": run.id_run})
            df_list.append(fields_dict)

        df = pd.json_normalize(df_list, max_level=max_level)
        ...
        for run in self.values():
            if name not in run.fields:
                ...
                if config or run.params:
                    raise InvalidInput(
                        "Trying to overwrite existing `config` or `params` via `args_field`."
                    )
    ...
    def execute(
        ...
        if options().get("execution.backend") == DEFAULT_BACKEND and options().get(
            "execution.loky_chdir"
        ):
            ...
            from .steps.chdir import chdir

            steps = [chdir()] + steps

        if not options().get("codelog.disable"):
            from .steps.codelog import codelog

            steps = [codelog(steps)] + steps

        # List of functions to execute.
        task_funcs = [
            run.execute_func(steps=steps, config=config, options=options())
            for run in self.values()
        ]
...
def normalize_runs(runs) -> list[Run]:
    """
