    ...
    def thread_main(self):
        ...
        while not self.terminate.is_set():
            self.sequence.append(
                **get_stats(path=path, interval=interval, percpu=percpu)
            )
            self.produced.set()
...
def get_stats(path: str = sep, interval: float = 1, percpu: bool = False):
    ...
    stats_net = {
        "net_recv_kbs": round(
            (net_end.bytes_recv - net_begin.bytes_recv) / interval / 1e3, 2
        ),
        "net_sent_kbs": round(
            (net_end.bytes_sent - net_begin.bytes_sent) / interval / 1e3, 2
        ),
    }

    # return stats_mem | stats_cpu | stats_disk | stats_net
    result = {}
    result.update(stats_mem)
    result.update(stats_cpu)
    result.update(stats_disk)
